...
#scan for enabled auth methods withing passed in namespace
function findmountpoint() {
VAULT_NAMESPACE=$1
for mount in $(vault_curl --request GET $VAULT_ADDR/v1/sys/auth|jq '.data | keys' |grep \"|cut -d\" -f2|sed 's/\///');
	do 
        if echo "$mount"|grep -q userpass
	then
        	userpass $1 $mount 
	elif echo "$mount"|grep -q ldap
	then
		ldap $1 $mount
	elif echo "$mount"|grep -q token
	then
		token $1 $mount
	elif echo "$mount"|grep -q approle
	then
		approle $1 $mount
