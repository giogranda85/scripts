...
function kernelsettings () {
	...
	if [ $tmp -lt 32000 ] ; 
	then
		echo -e "WARNING - THE RECOMMENDED NUMBER OF FILE DESCRIPTORS IS BELOW 32K\nFile descriptor limits:\nHard limit is: `ulimit -Hn`\nSoft limit is: `ulimit -Sn`\n\n"
	elif [[ $tmp -gt 32000 ]] && [[ $tmp -lt 64000 ]];
	then	
		echo -e "WARNING - THE RECOMMENDED NUMBER OF FILE DESCRIPTORS IS BETWEEN 32K & 64K\nFile descriptor limits:\nHard limit is: `ulimit -Hn`\nSoft limit is: `ulimit -Sn`\n\n"
	
	else
		echo -e "Number of file descriptors is above 64k and inline with recommended practice"
	fi
...
#consul and vault should be running on the same hosts, but the consul agents on the hosts shouldn’t be servers. There’s a flag in the consul config you can look for

#there’s a typo in the echo for that check

# you might want some clauses in the storage check that do something if they’re running a non-Consul or non-Raft storage engine

# probably want to throw a warning if ulimit -u and ulimit -n are set below 32k each

# check vm.swappiness: you want this set low if you have mlock because you don’t want Vault to swap

# check shell history: you want this disabled 

# validate that Vault can use mlock without needing to run as root (ie: look for the presence of a LimitMEMLOCK=inifinity in the systemd unit

# validate that vault and consul aren’t being run as root
